...
export const foodDefinitionSchema = z
  .object({
  ...
  .refine(
    (data) => data.availablePreparations.includes(data.defaultPreparation),
    {
      message: 'defaultPreparation must be included in availablePreparations',
      path: ['defaultPreparation'],
    },
  );
